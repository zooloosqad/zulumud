...
ACMD(do_gsay)
{
  ...
  if (!GROUP(ch)) {
    send_to_char(ch, "You are not in a group!\r\n");
    return;
  }
  if (!*argument)
    send_to_char(ch, "Group-say what?\r\n");
  else {
...
ACMD(do_write)
{
  ...
  if (!*papername) {
    ...
    send_to_char(ch, "Write?  With what?  ON what?\r\n");
    return;
  }
  if (*penname) {
    /* Nothing was delivered. */
